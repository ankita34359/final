...
export default function Practice() {
  ...
  try {
    const payload = {
      username: currentUser?.username || "ankita",
      xpChange: xp,
      reason: "practice_session"
    };

    const res = await fetch("http://localhost:8000/api/xp/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    if (data.total_xp !== undefined) {
      setTotalXP(data.total_xp); // update XP in frontend
    }

    // Refresh leaderboard UI if needed
